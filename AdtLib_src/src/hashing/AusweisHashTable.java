...
public class AusweisHashTable extends HashTable<FlensburgPerson> {
    ...
    public boolean add(FlensburgPerson element) {
        ...
        int idx = Math.floorMod(element.ausweisHashCode(),a.length);
        ListIterator<FlensburgPerson> ite = a[idx].listIterator();
        //noinspection WhileLoopReplaceableByForEach
        while ( ite.hasNext() ) {
            if ( ite.next().ausweisEquals(element.getAusweis()) ) return false;
        }
        a[idx].add(element);
        size++;
        modCtr++;
     ...
     * @param ausweis The element to be removed.
     * @return true if the element could be found and deleted from the bucket list,
     *              otherwise false.
     */
    public FlensburgPerson remove(Integer ausweis) {
        int idx = Math.floorMod(ausweis.hashCode(),a.length);
        ListIterator<FlensburgPerson> ite = a[idx].listIterator();
        FlensburgPerson test;
        while ( ite.hasNext() ) {
            test = ite.next();
            if ( test.ausweisEquals(ausweis)) {
                ite.remove();
                size--;
                return test;
            }
     ...
     * @param ausweis The element to be checked for presence in the table
     * @return true If the element could be found, otherwise false
     */
    public FlensburgPerson search(Integer ausweis) {
        int idx = Math.floorMod(ausweis.hashCode(),a.length);
        ListIterator<FlensburgPerson> ite = a[idx].listIterator();
        FlensburgPerson test;
        while ( ite.hasNext() ) {
            test = ite.next();
            if ( test.ausweisEquals(ausweis)) return test;
        }
        return null;
    }
    ...
    public int hashCode() {
        ...
        while ( ite.hasNext() ) {
            ...
            if ( element != null ) h += element.ausweisHashCode();
        }
        return h;
    }
}
